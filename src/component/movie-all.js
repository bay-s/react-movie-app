...
class MoviesPage extends React.Component{
    ...
    async componentDidUpdate(){
        const stream = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=0ccbee0a69447c2b1bd0090bf76b0358&watch_region=US&with_watch_monetization_types=flatrate&page=${this.state.pages}`)
        
        const streamTv = await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=0ccbee0a69447c2b1bd0090bf76b0358&watch_region=US&with_watch_monetization_types=flatrate&page=${this.state.pages}`)
            const tvRes = await streamTv.json()
            const movieRes = await stream.json()
            if(movieRes ){
                this.setState(prev => {
